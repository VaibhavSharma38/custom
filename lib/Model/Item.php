...
class Model_Item extends \xepan\hr\Model_Document{
	...
	function init(){
		...
		$item_j->addField('hsn_sac');
		// $item_j->addField('is_packaged');
	...
	function page_duplicate($page,$acl=true){
		...
		if($form->isSubmitted()){
			...
			$sku_item = $this->add('xepan\custom\Model_Item');
	...
	function updateChild($fields, $replica_fields){
		...
		$childs = $this->add('xepan\custom\Model_Item')->addCondition('duplicate_from_item_id',$this->id);
	...
	function getChildItem(){
		...
		$child_items = $this->add('xepan\custom\Model_Item')->addCondition('duplicate_from_item_id',$this->id)
