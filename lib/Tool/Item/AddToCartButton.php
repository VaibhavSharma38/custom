...
class Tool_Item_AddToCartButton extends \View{
	...
	function init(){
		...
		$this->app->stickyGET(' category_id');
	...
	function setModel($model){
		...
		if($this->options['amount_group_in_multistepform'] && !isset($fieldset)){
			...
			$price_view = $fieldset->add('View',null,null,['view/tool/add-to-cart-button','amount_section']);
		...
		foreach ($custom_fields as $custom_field) {
			...
			if(strtolower($custom_field['display_type']) === "dropdown" ){
				$field = $fieldset->addField('xepan\base\Form_Field_NoValidateDropDown',"f_".$count,'Available Size (ft.)');
				...
				$category_m->tryLoadBy('slug_url',$_GET['parent_category_code']);
				
				$parent_category_id = $category_m->id;	
				$cf_value_m->addExpression('stock_exist')->set(function($m,$q)use($parent_category_id){
		...
		if($form->isSubmitted()){						
			...
			if($form->isClicked($addtocart_btn)){
				if($form['qty'] < 1 || is_nan($form['qty']) || ($form['qty'] != (int)$form['qty'])){
					$form->displayError('qty','Quantity must be a number > 0');
				}
				...
				foreach ($custom_fields as $custom_field) {
					...
					$it_m = $this->add('xepan\commerce\Model_Item');
					$it_m->tryloadBy('slug_url',$_GET['item_code']);

					$stock_m = $this->add('xepan\custom\Model_ItemStock');
					$stock_m->addCondition('item_id',$it_m->id);
					$stock_m->addCondition('size',$form[$field_name]);
					$stock_m->tryLoadAny();

					$cart_m = $this->add('xepan\commerce\Model_Cart');

					$total_qty = $form['qty'];
					foreach ($cart_m as $cart_item) {
						$department_m = $this->add('xepan\hr\Model_Department');
						$department_m->loadBy('name','Company');

						$generic_m = $this->add('xepan\commerce\Model_Item_CustomField_Generic');
						$generic_m->loadBy('name','Size');

						$custom_field_array = $cart_item['custom_fields'];
						$custom_size = $custom_field_array[$department_m['id']][$generic_m['id']]['custom_field_value_name'];

						$i_m = $this->add('xepan\custom\Model_Item');
						$i_m->tryLoadBy('slug_url',$_GET['item_code']);

						$item_id_from_slug = $i_m->id;
						if(($cart_item['item_id'] == $item_id_from_slug) AND ($custom_size == $form[$field_name]))
							$total_qty += $cart_item['qty'];
					}

					if(!$stock_m->loaded())
						return $form->js()->univ()->errorMessage('Not in stock')->execute();

					if($stock_m['current_stock'] < $total_qty)
						return $form->js()->univ()->errorMessage('maximum quantity allowed is '.$stock_m['current_stock'])->execute();
				}
				...
				$commerce_item = $this->add('xepan\custom\Model_Item');
				$commerce_item->tryLoadBy('slug_url',$_GET['item_code']);
	...
	function defaultTemplate(){
		return ['view/tool/add-to-cart-button'];
	}
}